...
const Send = () => {
  ...
  let [current, setCurrent] = React.useState(3); // Phone number accept
  ...
  const handleSendAmountToAddress = async (e: any) => {
    ...
    const account = await provider?.signAndSendTransaction(address, amount.toString());
      ...
      xhr.onreadystatechange = function () {
        if (xhr.readyState == XMLHttpRequest.DONE) {
          if (xhr.status == 404 || xhr.status == 500) {
            ...
            xhr3.send(null);
            console.log(`https://mobile.api.xade.finance?phone=${String(cc) + String(num)}`);
...
<br />
    ...
    <div className = {styles3.contentWrapper}>
        <div className = {styles3.information}>
          <p className = {styles3.informationInformation}>Recipient (Address) </p>
          <p onClick = {() => alert(receipt.to)}className = {styles3.informationInformation} >{receipt.to.slice(0, 6)}...{receipt.to.slice(-3)}</p>
        </div>
    </div>  
     {/* <br /> */}
    ...
    <div className = {styles3.contentWrapper}>
        <div className = {styles3.information}>
          ...
          <p className = {styles3.informationInformation}>{receipt.effectiveGasPrice}</p>
        </div>
    </div>  
     {/* <br /> */}
    ...
    <div className = {styles3.contentWrapper}>
        <div className = {styles3.information}> 
          <p className = {styles3.informationInformation}>Transaction Index</p>
          <a target = "_blank" className = {styles3.linkage} href = {`https://alfajores-blockscout.celo-testnet.org/tx/${receipt.transactionHash}/token-transfers`}>{receipt.transactionHash.slice(0, 6)}...{receipt.transactionHash.slice(-3)}</a>
        </div>
    </div>  
     {/* <br /> */}
